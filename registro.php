...
try {
	...
	$Trabajado_Por  = $_POST['jsonObj'] [25]['value']; 	
	...
	//}else{
		...
		//}else{			
			if ($Accion_LC == 'Activar' and $Accion_CA == 'Activar' and $Accion_SG == 'Activar' and $Accion_Photo == 'Activar' && $Tipo_CA <> 'Utilitario/Furgon' ){	
				$Status_Final = 'OK';	
					}else{
						if ($Tipo_CA == 'Furgon' || $Tipo_CA == 'Utilitario'){
							$Status_Final = 'Reject';
...
$sql = 	    "UPDATE datos_cca SET   Licencia_LC=?, NomApell_LC=?, Sexo_DP=?, FechNac_LC=?, Vencimiento_LC=?, Accion_LC=?, Plate_CA=?, Prod_CA=?, ModVeh_CA=?, Tipo_CA=?, Uso_CA=?, VigCed_CA=?,Titular_CA=?, Autorizado_CA =?, Radicado_CA=?, Accion_CA=?, Vigencia_SG=?, Dominio_SG=?,  Anio_CA=?, Accion_SG=?,  Status_Final=?, Trabajado_Por=?, Accion_Photo=?,   Fecha_Hora=? WHERE ID=?";
$stmt= $conn->prepare($sql);
					$stmt->execute([$Licencia_LC,  $NomApell_LC,   $Sexo_DP,  $FechNac_LC,  $Vencimiento_LC, $Accion_LC,    $Plate_CA, $Marca_CA,  $ModVeh_CA,   $Tipo_CA, $Uso_CA, $VigCed_CA,$Titular_CA,   $Autorizado_CA,   $Radicado_CA,   $Accion_CA,  $Vigencia_SG,  $Dominio_SG,   $Anio_SG, $Accion_SG,   $Status_Final,  $Trabajado_Por,   $Accion_Photo,    $FechaUsuario, $ID]);
